---
name: nv-skill-updater
description: Pushes a locally installed NanoGiants skill back to the private GitHub repository https://github.com/NanoVerse-GmbH/skills. Use this skill whenever the user wants to publish, upload, sync, update, or push a local skill to the NanoGiants skills repo — also triggered by phrases like "push skill to repo", "skill hochladen", "skill ins repo pushen", "änderungen am skill veröffentlichen", "skill updaten im repo", "sync skill with remote", "update nv skill in github", "meine skill-änderungen pushen", "skill contribution einreichen", or anything that involves writing a skill back to the NanoVerse-GmbH/skills repository. Trigger proactively when the user has modified a locally installed NanoGiants skill and wants to share or persist the changes.
license: MIT
metadata:
  author: NanoGiants
  version: '1.0.0'
...
# NanoGiants Skill Updater

Pushes a locally installed skill back to the private `https://github.com/NanoVerse-GmbH/skills` repository.

**Hardcoded constants — never ask the user for these:**
- **Repo HTTPS**: `https://github.com/NanoVerse-GmbH/skills`
...
Silently scan all known install locations for locally installed NanoGiants skills. Do this via bash — never ask the user where their skills are.
...
```
Locally installed NanoGiants skills:

1. nv-idea-to-epic          → ~/.config/opencode/skills/nv-idea-to-epic/       [global / opencode]
2. nv-idea-to-epic          → .opencode/skills/nv-idea-to-epic/                [project-local / opencode]
3. nv-epic-to-user-stories  → ~/.config/opencode/skills/nv-epic-to-user-stories/ [global / opencode]
4. nv-story-to-implementation → .claude/skills/nv-story-to-implementation/     [project-local / claude]
...
```
❌ No NanoGiants skills found on this machine.
...
Install skills first with the nv-skill-installer skill.
```
...
```bash
TMPDIR="/tmp/nv-skills-update-$(date +%s)"
mkdir -p "$TMPDIR"
cd "$TMPDIR"
git init
```
...
Push these changes to https://github.com/NanoVerse-GmbH/skills? (y/n/force)
```
...
gh api repos/NanoVerse-GmbH/skills/commits/master --jq '.sha' 2>/dev/null
```
...
This usually means someone else pushed to the repo since you last synced.
...
```bash
gh api repos/NanoVerse-GmbH/skills/contents/README.md --jq '.content' | base64 --decode > "$TMPDIR/README.md"
README_SHA=$(gh api repos/NanoVerse-GmbH/skills/contents/README.md --jq '.sha')
```
...
gh api --method PUT repos/NanoVerse-GmbH/skills/contents/README.md \
...
📄  README.md updated: added <skill-name> to the "Vorhandene Skills" table.
    → https://github.com/NanoVerse-GmbH/skills/blob/master/README.md
```
...
Pushed:
  • nv-idea-to-epic          → https://github.com/NanoVerse-GmbH/skills/tree/master/skills/nv-idea-to-epic
  • nv-epic-to-user-stories  → https://github.com/NanoVerse-GmbH/skills/tree/master/skills/nv-epic-to-user-stories

Skipped (no changes):
  • nv-create-testplans

Skipped (user declined):
  • nv-story-to-implementation

🔗 View on GitHub: https://github.com/NanoVerse-GmbH/skills
```
...
| Situation | Action |
|-----------|--------|
| No skills found locally | Show install instructions and suggest `nv-skill-installer` |
| Repo not accessible after auth | "Cannot access https://github.com/NanoVerse-GmbH/skills — check your GitHub account has write access to this private repo" |
