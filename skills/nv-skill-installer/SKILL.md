...
description: >
  ...
  "install all nv skills globally", or any mention of installing/updating skills from the nanogiants repo.
  Trigger proactively when the user mentions nv-skills, NanoGiants skills, or the nanoverseai-skills repository.
---
...
> - **Global** → available in all your projects (recommended for shared team skills)
...
- Skill name (e.g. `nv-idea-to-epic`)
- Directory (e.g. `skills/nv-idea-to-epic/`)
...
```
Available NanoGiants Skills:

1. nv-idea-to-epic          — Interview → vollständiges Jira Epic
2. nv-epic-to-user-stories  — Epic → User Stories mit ACs
3. nv-create-testplans      — Story → Testfall-Suite in Jira
4. nv-story-to-implementation — Story → Implementierungsplan + Jira Tasks
5. nv-story-to-optimization — Post-Implementierung: Review & Optimierung
...
```bash
TMPDIR="/tmp/nv-skills-install-$(date +%s)"
mkdir -p "$TMPDIR"
```
...
```
⚠️  nv-idea-to-epic already exists at <destination>
...
```
✓ nv-idea-to-epic is already up to date. Skipping.
```
...
Installed:
  • nv-idea-to-epic        → ~/.config/opencode/skills/nv-idea-to-epic/
  • nv-epic-to-user-stories → ~/.config/opencode/skills/nv-epic-to-user-stories/
