---
name: nv-story-to-implementation
description: >
...
Phase 4 besteht aus drei Schritten die sequenziell ausgeführt werden.

### Schritt 0: Branch sicherstellen

Bevor die Datei gespeichert wird, muss der richtige Feature-Branch aktiv sein.
Die Datei `tasks/IMPL_<STORYKEY>.md` soll direkt im Feature-Branch landen — nicht auf `main`.

**Branch-Name** wird aus Story-ID + Titel gebildet:
Titel in Kleinbuchstaben, Leerzeichen → `-`, Sonderzeichen entfernen, max. 5 Wörter.
Beispiel: "SCRUM-47 – Theme Preference für User implementieren" → `feature/SCRUM-47-theme-preference-user-implementieren`
...
#### Fall A: Bereits auf Feature-Branch mit Story-ID

```
✓ Branch "feature/SCRUM-47-..." ist aktiv — fahre fort.
```

#### Fall B: Auf main / develop / anderem Branch → Nachfragen

```
Du bist aktuell auf Branch "<aktueller-branch>".
Die Plandatei sollte im Feature-Branch für diese Story gespeichert werden.

Soll ich wechseln zu / erstellen: feature/SCRUM-47-theme-preference-user-implementieren?
  → Ja: Branch wird angelegt (Basis: main) oder gewechselt falls vorhanden
  → Nein: Datei wird trotzdem hier gespeichert (nicht empfohlen)
```

Bei Bestätigung — Branch vorhanden:
```bash
git checkout feature/SCRUM-47-theme-preference-user-implementieren
```

Bei Bestätigung — Branch neu anlegen:
```bash
git checkout main && git pull
git checkout -b feature/SCRUM-47-theme-preference-user-implementieren
```

Bestätigung: `✓ Branch "feature/SCRUM-47-theme-preference-user-implementieren" aktiv`

#### Jira-Status → "In Progress"

Direkt nach Branch-Wechsel/-Erstellung:

1. Transitions laden: `getTransitionsForJiraIssue(<STORY-KEY>)`
2. Transition "In Progress" anwenden.
3. Bestätigung: `✓ <STORY-KEY> ist jetzt "In Progress"`
...
#### Schritt 3: Jira-Status → "In Review" + Abschlusskommentar

Nach erfolgreicher Implementierung und Anlage der Tasks:

1. Transitions laden: `getTransitionsForJiraIssue(<STORY-KEY>)`
2. Transition "In Review" anwenden.
3. Abschlusskommentar erstellen via `addCommentToJiraIssue`:

```
## Implementierung abgeschlossen

**Story:** <STORY-KEY> – <Titel>
**Branch:** feature/<STORY-KEY>-<story-titel>
**Datum:** <aktuelles Datum>

### Implementierungsplan
- Plan-Datei: `tasks/IMPL_<STORYKEY>.md`
- Angelegte Tasks: <n> Tasks, <n> Subtasks

### Zusammenfassung
<2-3 Sätze: Was wurde umgesetzt, welche Architekturentscheidungen wurden getroffen.>

### Nächster Schritt
Surface-Review: Oberfläche und UX auf Basis des Implementierungsplans prüfen.
```

4. Bestätigung:
```
✓ <STORY-KEY> ist jetzt "In Review"
✓ Jira-Kommentar wurde erstellt
✓ Branch: feature/<STORY-KEY>-<story-titel>
